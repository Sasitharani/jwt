...
const updateVotes = async (username, email, fetchVotesDetails) => {
    try {
        ...
        Swal.fire('Error', error.response?.data || 'An error occurred while updating votes.', 'error');
    }
};
...
const VotesManager = () => {
    ...
    const fetchVotesDetails = async () => {
        try {
            ...
            Swal.fire('Error', error.response?.data || 'An error occurred while fetching votes details.', 'error');
        }
    };
